...
const GemCard: React.FC<GemCardProps> = ({ gem }) => {
  const {addGemToWheel} = useWheelContext()
      ...
      >
        ...
        <div className="absolute left-1/2 transform -translate-x-1/2 -top-28 hidden group-hover:flex flex-col 
          ...
          <h3 className="text-md font-semibold">
            <a href={`https://www.poewiki.net/wiki/${encodeURIComponent(gem.name)}`}target="_blank" className="hover:cursor-pointer mt-1 px-2 py-1 bg-gray-800 text-white rounded hover:bg-gray-700 ">{gem.name}</a>
          </h3>
          {gem.category === 'active' && (<div>
            <button onClick={handleAddToWheel} className='mt-1 px-2 py-1 bg-gray-800 text-white rounded hover:bg-gray-700 '>Add to "The Wheel"</button>
          </div>
          )}
